...
Replication Queue is periodically blocked for one of our AEMaaCS customers. This article describes the workaround suggested by the Engineering Team that can be performed in the UI using Admin permissions.
...
One of our AEMaaCS Customers replication queue periodically blocked due to the javax.jcr.NamespaceException: Unknown namespace prefix: ns22_ exception.
...
Unblocking the queue requires simply deleting the request from the golden-publish distribution queue UI. Please suggest the customer to perform the below steps.
