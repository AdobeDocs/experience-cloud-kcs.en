...
First,  use the oak run tool, which is a runnable jar1 that contains everything you need for a simple Oak installation and performing oak-related operations.
...
1. Perform a checkpoint cleanup by listing checkpoints using below. If there are more than one checkpoint, then clean them up:
    ...
    ```
2. Run an offline compaction.  If you do not know how to run offline compaction then see [here](https://gist.github.com/andrewmkhoury/0b1fe4d8b619178ff87b).
3. Start the server wait for indexing completion.
