...
description: Learn about the Adobe Experience Manager issue where the message is not internationalized.
...
The issue is a message which is not internationalized. The error message, "Invalid date - The value must be of date datatype" is in English even if it should be in German, which is the customer’s browser language.
