...
However, the cloudmanager user exists and manually uploading assets on the author works as expected.

The assets performance test is working for 30 minutes then suddenly fails in a repeated manner while the test should stop after 30 minutes - `[` 1`]`
...
Checking the internal Azure function logs it was found the request that had the result 404 Not Found.

The GET to the URL from the "message" field returned 404 `[` 1`]` .

The failure in this case was caused by the fact that following Adobe's recommendation, querybuilder was blocked at the dispatcher level.

However, Cloud Manager needs the querybuilder to be allowed on the dispatcher for the cloudmanager user, as during the asset testing it retrieves the processing time.
