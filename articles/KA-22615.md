...
The customer is developing a website which should retrieve data from their AEM instances using AEM headless solution.

To set up the endpoint, the tutorial in `[` 1`]`  was followed.

After publishing all the content fragments, all the models, content, references, assets and queries as described in `[` 2`]` , when trying to retrieve any content fragment from the publish instance, a 404 error is received preventing the content to be read from the publisher.
