...
The customer followed the steps in `[` 1`]`  to enable Smart tagging (Smart content service) creating an AIO project and also a Legacy cloud configuration in AEM.

After completing the smart tagging integration they tried creating a Smart Tag training for their custom tags - `[` 2`]` . However, every training failed with an error `[` 3`]`  present in the logs.
...
The documentation followed by the customer to integrate AEM with Smart Content is for AEM on premise version 6.5.
...
After removing the legacy cloud configuration the smart training for custom tags completed successfully.
