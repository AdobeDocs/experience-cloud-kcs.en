...
article-published-date: "9/13/2023 10:30:11 AM"
version-number: 1
...
The customer has the AEM environment set up with the 2 authors, Publishers and Dispatchers. There is one Active and one Standby author and the publishers and dispatchers are in high availability.
...
After this operation the customer was able to access the secondary one. When everything was well the authors were switched back like before.
...
4. Navigate to OSGi `>`  Bundles and confirm bundle`<` ID`>`  for "com.adobe.granite.crypto.file"

5. Take a backup of "/crx-quickstart/launchpad/felix/bundle`<` ID`>` /data"

6. Paste the two copied files from author1 to the same path in author2 replacing the old ones

7. Restart the target com.adobe.granite.crypto bundle or the entire AEM instance.
