...
description: Learn to fix the issue where an error occurs: "The end_aem_createsegmentstore property is missing" in the Adobe Experience Manager.
...
article-created-date: "11/28/2023 1:56:22 PM"
article-published-by: Amol Mahajan
article-published-date: "11/28/2023 1:59:47 PM"
version-number: 6
article-number: KA-22091
dynamics-url: "https://adobe-ent.crm.dynamics.com/main.aspx?forceUCI=1&pagetype=entityrecord&etn=knowledgearticle&id=079f62e7-f58d-ee11-8179-6045bd006149"

---
# Pipeline Error: "The end_aem_createsegmentstore property is missing"


If the pipeline fails with an error: "The end_aem_createsegmentstore property is missing" in Adobe Experience Manager, follow the workaround given in this article to fix the issue.
...
The <b>Continuous Integration</b> and <b>Continuous Deployment</b> <b>(CI/CD)</b> pipeline keeps failing during the *Deploy to Development* stage.
<br>The deploy logs did not show any error due to the code base, but the following errors are present:<br> <br>`[` 1`]`   
...
Checking the pipeline, it could be seen that it fails with the error *errorCode*: *BLOCKED_QUEUE*.

This means that the replication queue is blocked, hindering the installation of additional mutable content.

In the logs, the errors <b>`[` 1`]` </b> are present during the deployment timeframe. Most likely the node - <b>`[` 2`]` </b> is not present on the publisher and/or preview server and the replication fails.

After clearing the replication queue, the pipeline deployment was successful.
