...
After creating the SAML 2.0 configuration to enable SSO login on their production Author the customer received an error message from the AEM side after the Azure provider performed the login and retrieved the SAML token, to perform the authentication on the AEM side.<br><br> <br><br>The error received was `[` 1`]` <br><br> <br><br><b>`[` 1`]`  \*ERROR\* `[` qtp1558388453-153215`]`  com.adobe.granite.auth.saml.SamlAuthenticationHandler SAML response parameter was not provided or invalid.</b>
...
Error 403 usually appears if the Apache Sling Referrer Filter is not enabled. Checking the configuration for the Apache Sling Referrer Filter the "Allow empty" option was not checked and there was also no host added into "Allow Hosts"
...
After completing these required steps - check "Allow Empty" and the adding the hosts that represent the IdP provider to the "Allow Hosts" the login was successful.



NOTE:

The IDP URL must be added to the Apache Sling Referrer Filter configuration without including the protocol - e.g. aem-sso-saml instead of https://aem-sso-saml
