...
article-created-date: "6/18/2024 10:29:15 AM"
article-published-by: Utkarsh Jha
article-published-date: "6/18/2024 10:52:04 AM"
version-number: 11
article-number: KA-17477
dynamics-url: "https://adobe-ent.crm.dynamics.com/main.aspx?forceUCI=1&pagetype=entityrecord&etn=knowledgearticle&id=aec62996-5d2d-ef11-840b-6045bd06eea5"
...
Apache Webserver doesn't actually care what the file extension is of a file when targeting it with an include or include optional statement. Naming them properly with names that eliminate conflicts and confusion helps a ton. Names used describes the scope of where the file is applied, it makes life easier. If, everything is named .conf this gets really confusing. Avoid poorly named files and extensions.
...
| --- | --- | --- |
| `<` FILENAME`>` .conf | /etc/httpd/conf.d/ | A default Enterprise Linux install uses this file extension and includes folder as a place to override settings in httpd.conf and allow you to add additional functionality at a global level in Apache. |
...
| `<` FILENAME`>` _whitelist.rules | /etc/httpd/conf.d/whitelists/ | \*_ipwhitelist.rules files are included from inside the \*.vhost files. It contains IP regex or allows deny rules to allow IP whitelisting. If you're trying to restrict viewing of a virtual host based on IP addresses you'll generate one of these files and include it from your \*.vhost file |
...
When following the naming convention, you can avoid some pretty easy to make mistakes that can have catastrophic results. Let's cover a few examples.
...
<b>How does this typically becomes an issue?</b>
...
The file extensions are the same and use auto-included extension that Apache auto includes any .conf files in many of it's default folders.

<b>How this typically becomes an issue?</b>

If the vhost file with the extension of .conf is put in the /etc/httpd/conf.d/ folder it tries to load it into memory on Apache which is typically ok, but if the rewrite rules file with the extension of .conf gets placed in the `/etc/httpd/conf.d/` folder, it gets auto-included and apply globally causing confusing and undesired results.
...
While applying the standard, some additional file types and includes of our own are added.
