...
description: "Explore the process of cache flushing and content activation, ensuring seamless updates and optimal performance in your system."
...
This document explains how flushing occurs and the mechanism that executes cache flushing and invalidation.
...
- Second is the reverse agent.  This is optional and is setup to check on each publisher's outbox to see if there is new content to pull into the author as a reverse replication activity
...
On the <b>Transport</b> tab, you can see the URI being set to point the IP address of the dispatcher that will receive the flush requests. The path `/dispatcher/invalidate.cache` isn't how the module determines if it's a flush; it's only an obvious endpoint you can see in the access log to know it was a flush request. On the <b>Extended</b> tab, we'll go over the things that are there to qualify that this is a flush request to the dispatcher module.
...
- <b>Host</b>    
    This is where the Host Header gets spoofed to target a specific `<` VirtualHost`>`  that is configured on the dispatcher Apache web server (/etc/httpd/conf.d/enabled_vhosts/aem_flush.vhost). It's a hard coded value that matches an entry in the aem_flush.vhost file's ServerName or ServerAlias


![](assets/___713c2079-762a-ef11-840b-6045bd0298d4___.png)

On the <b>Triggers</b> tab we'll take note of the toggled triggers we use and what they are
...
The stat file level setting indicates how many folders deep the module will traverse and update a .stat file. The .stat file is empty it's nothing more than a filename with a date stamp and could even be created manually but running the touch command on the command line of the dispatcher server.
...
Setting this file level too low can cause a flush request to clear more than was intended. This in turn would cause the cache to churn more often with less requests being served from cache and can cause performance issues.

<b>Note:</b> Set the statfilelevel at a reasonable level.  Look at your folder structure and make sure it's setup to allow concise flushes without having to traverse too many directories. Test it and make sure it fits your needs during a performance test of the system.

A good example is a site that supports languages. The typical content tree would have the following directories: `/content/brand1/en/us/`
...
2. If the .stat file is newer than the requested file, it deletes the cached content and fetches a new one from AEM and caches that. Then it serves the content
3. If the .stat file is older than the requested file it then knows the file is fresh and can serve the content.


<b><u>Cache Handshake: Example `[` 1`]` </u></b>
...
<b><u>Cache Handshake: Example `[` 2`]` </u></b>
...
Documentation is all here for the full set of configuration options:[Configuring AEM Dispatcher Guide](https://experienceleague.adobe.com/en/docs/experience-manager-dispatcher/using/configuring/dispatcher-configuration#configuring-dispatcher_configuring-the-dispatcher-cache-cache)

We will want to highlight a few of them that pertain to cache flushing.
...
You specify the directory in which you want the dispatcher to populate and manage as a cache directory.

<b>Note: </b>This directory should match the apache document root setting for the domain your web server is configured to use.
...
The `/statfileslevel` set at the following number with the document root of `/var/www/html/` would have the following results when flushing `/content/dam/brand1/en/us/logo.jpg`
...
<b>Note:</b>Keep in mind that when the timestamp handshake happens it looks for the closest .stat file.

Having a .stat file level 0 and a stat file only at `/var/www/html/.stat` means that content that lives under `/var/www/html/content/dam/brand1/en/us/` would look for the nearest .stat file and traverse up 5 folders to find the only .stat file that exists at level 0 and compare dates to that. Meaning that one flush at that high of a level would essentially invalidate all items cached.
