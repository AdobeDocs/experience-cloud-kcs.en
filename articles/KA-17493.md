...
description: Explore the intricate process of cache flushing and content activation, ensuring seamless updates and optimal performance in your system.
solution: Experience Manager
product: Experience Manager
applies-to: Experience Manager
keywords: KCS, Cache flushing mechanism, Dispatcher workflow, Replication agents, Activation process, Flush request handling, Serialization type, Dispatcher configuration, HTTP method for flush requests, Stat file structure, Timestamp handshake, Cache handshake examples, Farm file settings
...
Explore the nuances of cache flushing mechanisms and content activation workflows, vital for maintaining system efficiency and data integrity.
...
This document gives guidance on how flushing occurs and explains the mechanism that executes cache flushing and invalidation.
<br>How it works<br>
<b>Order of Operation</b>

The typical workflow is best described when content authors activate a page, when publisher receives the new content it triggers a flush request to the Dispatcher as shown in the following diagram:![](assets/___636e3fa8-7c2d-ef11-840b-6045bd006704___.png)

This chaining of events, highlights that we only flush items when they are new or have changed.  This ensures that content has been received by the publisher before clearing the cache to avoid race conditions where the flush could occur before the changes are to be picked up from the publisher.

### <b>Replication Agents</b>

On author, there is a replication agent configured to point at the publisher that when something is activated it triggers to send the file and all of its dependencies to the publisher.

When the publisher receives the file, it has a replication agent configured to point at the Dispatcher that triggers on the on-receive event.  It then serializes a flush request and post it to the Dispatcher.
...
There are typically 1 or 2 replication agents configured on the author for each publisher that they replicate content to.

The first is the standard replication agent that pushes content activations to.

The second is the reverse agent.  This is optional and is setup to check on each publisher's outbox to see if there is new content to pull into the author as a reverse replication activity

#### PUBLISHER REPLICATION AGENT

Here is an example screenshot of a configured standard flush replication agent:
...
On the transport tab you can see the URI being set to point to the IP address of the Dispatcher that receives the flush requests.  The path /dispatcher/invalidate.cache isn't how the module determines if it's a flush it's only an obvious endpoint you can see in the access log to know it was a flush request.  On the Extended tab, go over the things that are there to qualify that this is a flush request to the Dispatcher module.
...
- Host
    ...
    On the triggers tab, take note of the toggled triggers and use and what they are.


- <b>Ignore Default</b>

    This is enabled so the replication agent isn't triggered upon a page activation.  This is something that when an author instance were to make a change to a page would trigger a flush.  Because this is a publisher, don't trigger off that type of event.

    
- <b>On Receive</b>

    When a new file is received, trigger a flush.  So when the author sends us an updated file, trigger and send a flush request to the Dispatcher.

    
- <b>No Versioning</b>

    Check this to avoid the publisher from generating new versions because a new file was received.  Just replace the file we have and rely on the author to keep track of the versions instead of the publisher.


Now, if you look at what a typical flush request looks like in the form of a curl command
<br> <br>
...
#### Here is a diagram of how this file structure looks when you've got items in the cache and have had a flush request sent and processed by the dispatcher module:

#### ![](assets/___736e3fa8-7c2d-ef11-840b-6045bd006704___.png)

STAT FILE LEVEL

Notice that in each directory there was a .stat file present.  This is an indicator that a flush has occurred.  In the example above the <b>stat file level</b> setting was set to <b>3</b> inside the corresponding farm configuration file.

The stat file level setting indicates how many folders deep the module traverses and updates a .stat file.  The .stat file is empty it's nothing more than a filename with a date stamp and could even be created manually but running the touch command on the command line of the dispatcher server.
...
Setting this file level too low can cause a flush request to clear more than was intended.  Which in turn would cause the cache to churn more often with fewer requests being served from cache and can cause performance issues.
...
In this example use a stat file level setting of 4.  This assures when you flush content that lives under the <b>us</b> folder that it won't cause the language folders to get flushed as well.
...
As you can see in this example the file is older than the .stat file and is removed and a fresh one pulled from AEM to replace it in the cache before being served to the end user who requested it.
...
Lets highlight of few of them that pertain to cache flushing
...
<br> 
This setting gauges how deep .stat files needs to be generated when a flush request comes in.
...
You refresh your page and see the old stuff and there is green light.
