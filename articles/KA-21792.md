...
<b>Issue/Symptoms</b>
<br>In the release 2023.02.11382, some improvements have been made in the GraphQL (Open-Source Query Language) implementation that might cause unexpected behavior in the application code, in case when it relied on the erroneous behavior of the older releases. 
<br>The below mentioned resolution helps achieve that Adobe Experience Manager (AEM) GraphQL now returns content with null values consistently across all filter operations. <br><br>
