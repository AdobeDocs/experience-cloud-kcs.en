...
Most Adobe Experience Cloud functionality that requires real-time response utilizes these regional nodes. The first call on a web page or mobile app to ID Service or Experience Edge determines which regional node to use. The location hints can be found in the response to these calls:
...
Once the closest regional node to the end user is determined, the region identifier passed to Analytics, Target, and AEP WebSDK calls going forward. In Analytics, it is passed as the aamlh query string parameter:
